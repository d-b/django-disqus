...
class WxrFeedType(feedgenerator.Rss201rev2Feed):
    ...
    def add_item(self, title, link, description, author_email=None,
        ...
        if categories:
            categories = [to_text(c) for c in categories]
        if ttl is not None:
        ...
        item = {
            ...
            'author_email': to_text(author_email),
            'author_name': to_text(author_name),
            'author_link': iri_to_uri(author_link),
            'pubdate': pubdate,
            'comments': comments,
            'unique_id': to_text(unique_id),
            'enclosure': enclosure,
            'categories': categories or (),
            'item_copyright': to_text(item_copyright),
            'ttl': ttl,
        }
        item.update(kwargs)
